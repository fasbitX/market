...
//$con =mysqli_connect('localhost','root','','cryptocompare') or die(mysqli_error());
...
foreach ($decoded->Data as $key => $value) {
		...
		if (in_array($value->Id, $list))
		{
			...
			//echo $coin_details->RAW->$coin_name->USD->MKTCAP;
			if($count==0){
				...
				'".$coin_details->RAW->$coin_name->USD->MKTCAP."',
				'".$coin_image."',
				'".$chart_image."')";
				...
				$sql=" UPDATE live_data set  name='".$coin_name."', 
						...
						market_cap='".$coin_details->RAW->$coin_name->USD->MKTCAP."',
